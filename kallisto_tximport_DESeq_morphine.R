...
setwd("C:/Users/mrzsjs1/OneDrive - The University of Nottingham/RNASeq/RNASeq_KNS42/morphine")
...
conditions = c('Morphine_treatment', 'Control')
...
#can look at expression of any gene - these are DARPP protein coding transcripts
plotCounts(DESeq_dataset, gene= 'ENST00000254079.9', intgroup='conditions')
plotCounts(DESeq_dataset, gene= 'ENST00000394267.2', intgroup='conditions')
plotCounts(DESeq_dataset, gene= 'ENST00000579000.5', intgroup='conditions')
plotCounts(DESeq_dataset, gene= 'ENST00000394265.5', intgroup='conditions')
plotCounts(DESeq_dataset, gene= 'ENST00000580825.5', intgroup='conditions')
plotCounts(DESeq_dataset, gene= 'ENST00000582680.5', intgroup='conditions')
plotCounts(DESeq_dataset, gene= 'ENST00000614012.4', intgroup='conditions')
...
annotated_df = left_join(updated_df1, annotation,
                         by=c("ensgene"="ensembl_transcript_id_version"))
...
colnames(updated_df3)[colnames(updated_df3)=="rowname"]<-"ensgene"
updated_df3

ent_gene = getBM(attributes = c('entrezgene_id'),
                 filters = c('ensembl_transcript_id_version'),
                 values = updated_df3$ensgene,
                 mart = ensembl99)
...
#save data
write.table(filter_df3, file='DET.csv', sep ="\t", row.names = TRUE)
